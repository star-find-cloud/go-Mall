...
func NewDeepseekService(dsAdapter *ds.DeepSeekAdapter, userRepo *repo.UserRepoImpl, productRepo *repo.ProductRepoImpl) *Deepseek {
	return &Deepseek{dsAdapter: dsAdapter, userRepo: userRepo, productRepo: productRepo}
}

// SuggestProductByUserTags 根据用户标签，获取与用户标签匹配的热门商品
func (s *Deepseek) SuggestProductByUserTags(ctx context.Context, userid int64, model domain.DeepseekModel) (<-chan string, error) {
	// 获取用户近期感兴趣的标签
...
func (s *Deepseek) SearchProductSuggest(ctx context.Context, searchMsg string, model domain.DeepseekModel) (<-chan string, error) {
