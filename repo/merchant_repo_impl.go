...
func NewMerchantRepo(db *sqlx.DB, cache *database.Redis) *MerchantRepo {
...
func (r *MerchantRepo) Create(ctx context.Context, merchant *domain.Merchant) (int64, error) {
	...
	result, err := r.db.ExecContext(ctx, sqlStr, merchant.UserID, merchant.Name, merchant.Phone, merchant.Email, merchant.Password, merchant.RealName, merchant.RealID, merchant.LicenseImageID, merchant.Tag, merchant.CateID, businessTypeJSON, merchant.Score, merchant.CreateAt, _const.StatusNotDeleted)
	if err != nil {
...
func (r *MerchantRepo) GetMerchantByID(cxt context.Context, id int64) (*domain.Merchant, error) {
	...
	err := r.db.GetContext(cxt, &merchant, sqlStr, id)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			applog.MySQLLogger.Error("GetMerchantByID", "err", err)
			return nil, err
...
func (r *MerchantRepo) GetMerchantByName(cxt context.Context, name string) (*[]domain.Merchant, error) {
	...
	sqlName := "%" + name + "%"
	err := r.db.SelectContext(cxt, &merchant, sqlStr, sqlName, 0)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
...
func (r *MerchantRepo) GetMerchantByEmail(ctx context.Context, email string) (*domain.Merchant, error) {
	...
	err := r.db.GetContext(ctx, &merchant, sqlStr, email)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
...
func (r *MerchantRepo) GetMerchantByPhone(ctx context.Context, phone string) (*domain.Merchant, error) {
	...
	err := r.db.GetContext(ctx, &merchant, sqlStr, phone)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
...
func (r *MerchantRepo) Update(ctx context.Context, merchant *domain.Merchant) error {
	...
	_, err = r.db.ExecContext(ctx, sqlStr, merchant.Name, merchant.OldName, merchant.Phone, merchant.Email, merchant.CateID, merchant.BusinessType, merchant.ID)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			applog.MySQLLogger.Error("GetMerchantByID", "err", err)
			return err
...
func (r *MerchantRepo) UpdateLicenseImage(ctx context.Context, merchantID, imageID int64) error {
	...
	now := time.Now().Unix()
	_, err := r.db.ExecContext(ctx, sqlStr, imageID, now, merchantID)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
...
func (r *MerchantRepo) Delete(ctx context.Context, id int64) error {
	...
	now := time.Now().Unix()
	_, err := r.db.ExecContext(ctx, sqlStr, _const.StatusDeleted, now, id)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			applog.MySQLLogger.Error("GetMerchantByID", "err", err)
			return err
...
func (r *MerchantRepo) IsExistsByID(ctx context.Context, id int64) (bool, error) {
	...
	err := r.db.QueryRowxContext(ctx, sqlStr, id).Scan(&exists)
	if err != nil {
...
func (r *MerchantRepo) IsExistsByEmail(ctx context.Context, email string) (bool, error) {
	...
	err := r.db.QueryRowxContext(ctx, sqlStr, email).Scan(&exists)
	if err != nil {
		return false, errors.New("failed to check if merchant exists")
	}
	return exists, nil
...
func (r *MerchantRepo) IsExistsByPhone(ctx context.Context, phone string) (bool, error) {
	...
	err := r.db.QueryRowxContext(ctx, sqlStr, phone).Scan(&exists)
	if err != nil {
		return false, errors.New("failed to check if merchant exists")
	}
	return exists, nil
