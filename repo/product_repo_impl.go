...
func (r *ProductRepoImpl) Create(ctx context.Context, product *domain.Product) (int64, error) {
	...
	//if err != nil {
	//	applog.AppLogger.Errorf("序列化图片id失败: %d", product.ImageID)
	//}
...
func (r *ProductRepoImpl) GetByID(ctx context.Context, id int64) (*domain.Product, error) {
	...
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			applog.MySQLLogger.Errorf("product not found (id: %d)", id)
...
func (r *ProductRepoImpl) GetByCateIDsAndHot(ctx context.Context, ids []int64) ([]domain.Product, error) {
	...
	for _, id := range ids {
		err := r.db.SelectContext(ctx, &products, sqlStr, id, id)
		if err != nil {
			if errors.Is(err, sql.ErrNoRows) {
				applog.MySQLLogger.Warnf("product not found (cateIDs: %v)", id)
...
func (r *ProductRepoImpl) SearchByMsg(ctx context.Context, msg string) ([]domain.Product, error) {
	...
	err := r.db.SelectContext(ctx, &products, sqlStr, "%"+msg+"%", "%"+msg+"%")
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
...
func (r *ProductRepoImpl) Update(ctx context.Context, product *domain.Product) error {
	...
	_, err := r.db.ExecContext(ctx, sqlStr, product.Title, product.SubTitle, product.CateID, product.Price, product.MarketPrice, product.Attr, product.Version, product.Desc, product.Content, product.UpdatedAt, product.IsBest, product.IsBooking, product.ProductTypeID, product.Status, product.ID)
	if err != nil {
		applog.AppLogger.Errorf("product repo error: %v", err)
