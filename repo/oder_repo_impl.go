...
func NewOrderRepo(db *sqlx.DB, cache *database.Redis) *OrderRepoImpl {
...
func (r *OrderRepoImpl) Create(ctx context.Context, order *domain.Order, orderItem *domain.OrderItem) error {
	...
	_, err := r.db.ExecContext(ctx, sqlStr, order.ID, order.UserID, order.OrderStatus, order.TotalPrice, order.PayPrice, order.CreatedAt, order.ShippingAddressID)
	if err != nil {
	...
	_, err = r.db.ExecContext(ctx, itemSqlStr, orderItem.OrderID, orderItem.ProductID, orderItem.ProductTitle, orderItem.UnitPrice, orderItem.Quantity, orderItem.Subtotal)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
...
func (r *OrderRepoImpl) GetByID(ctx context.Context, orderID int64) (*domain.Order, *domain.OrderItem, error) {
	...
	err := r.db.GetContext(ctx, order, sqlStr, orderID)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
	...
	err = r.db.GetContext(ctx, orderItem, ItemSqlStr, orderID)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
...
func (r *OrderRepoImpl) Delete(ctx context.Context, order *domain.Order) error {
	...
	_, err := r.db.ExecContext(ctx, sqlStr, order.ID)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
	...
	_, err = r.db.ExecContext(ctx, itemSqlStr, order.ID)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
