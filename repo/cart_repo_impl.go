...
func NewCartRepo(db *sqlx.DB, cache *database.Redis) *CartRepositoryImpl {
...
func (r *CartRepositoryImpl) Create(ctx context.Context, cart *domain.Cart) (int64, error) {
	// 生成 uid
	var cartUid, err = utils.GenerateUid()
	if err != nil {
		return 0, errors.New("generate uid failed")
	}
	var cartSqlStr = "insert into shop.cart(id, user_id, created_at) values(?,?,?)"

	_, err = r.db.ExecContext(ctx, cartSqlStr, cartUid, cart.UserID, time.Now().Unix())
	if err != nil {
...
func (r *CartRepositoryImpl) GetByUserID(ctx context.Context, userID int64) (*domain.Cart, error) {
	...
	err := r.db.GetContext(ctx, cart, sqlStr, userID)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
	...
	err = r.db.SelectContext(ctx, &cart.CartItems, itemSqlStr, cart.ID)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
...
func (r *CartRepositoryImpl) AddProduct(ctx context.Context, cartItem *domain.CartItemVO) error {
	...
	_, err = r.db.ExecContext(ctx, sqlStr, cartItem.CartID, cartItem.ProductID, cartItem.ProductTitle, cartItem.CreatePrice, cartItem.NowPrice, cartItem.ProductImageOss, specs, time.Now().Unix())
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
