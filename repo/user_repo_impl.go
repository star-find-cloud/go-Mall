...
func NewUserRepo(db *sqlx.DB, cache *database.Redis) *UserRepoImpl {
...
func (r *UserRepoImpl) GetByID(ctx context.Context, id int64) (*domain.User, error) {
	...
	if user.Status == _const.StatusDeleted {
		...
		_, err = r.db.ExecContext(ctx, sqlStr, _const.StatusNotDeleted, id)
		if err != nil {
			applog.AppLogger.Errorf("user repo error: %v", err)
			return nil, fmt.Errorf("failed to get user: %w", err)
		}
...
func (r *UserRepoImpl) GetPasswordByID(ctx context.Context, id int64) (string, error) {
	...
	err := r.db.GetContext(ctx, &password, sqlStr, id)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			applog.MySQLLogger.Warnf("user not found (id: %d)", id)
...
func (r *UserRepoImpl) UpdateImage(ctx context.Context, userID, imageID int64) error {
	...
	now := time.Now().Unix()
	_, err := r.db.ExecContext(ctx, sqlStr, imageID, now, userID)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
...
func (r *UserRepoImpl) CheckEmailVerificationCode(ctx context.Context, email string, verificationCode string) (bool, error) {
	// 从Redis缓存中获取存储的验证码
	storedCode, err := r.cache.GetReadCache().Get(ctx, email).Result()
	if err != nil {
		if errors.Is(err, redis.Nil) {
			applog.AppLogger.Warnf("email:%s err: %v", email, err)
			return false, err
		}
		applog.AppLogger.Errorf("获取验证码失败: %v", err)
	...
	// 如果验证成功，删除验证码防止重复使用
	if isValid {
		_, err = r.cache.GetWriteCache().Del(ctx, email).Result()
		if err != nil {
			applog.AppLogger.Warnf("删除已使用的验证码失败: %v", err)
		}
	}
...
func (r *UserRepoImpl) UpdateEmail(ctx context.Context, email string, userID int64) error {
	...
	now := time.Now().Unix()
	_, err := r.db.ExecContext(ctx, sqlStr, email, now, userID)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			applog.MySQLLogger.Warnf("user update email err: %v", err)
...
func (r *UserRepoImpl) UpdateUserTags(ctx context.Context, userID int64, tag int64) error {
	...
	_, err := r.db.ExecContext(ctx, sqlStr, tag, userID)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			applog.MySQLLogger.Warnf("user update tags err: %v", err)
...
func (r UserRepoImpl) GetUserTags(ctx context.Context, userID int64) ([]byte, error) {
	...
	err := r.db.QueryRowxContext(ctx, sqlStr, userID).Scan(&tags)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			applog.MySQLLogger.Warnf("user get tags err: %v", err)
